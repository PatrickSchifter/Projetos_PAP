...
from config import path_dir_tod, file, dest_path_qlik, ano_atual, meses_str, mes_atual, file_a, all_index, prov_index, \
    index_qlik, conversor_dt, fatiamento
...
df_mon['Número'] = df_mon.rename(columns={'Número': 'N.F'}, inplace=True)
df_qlik.columns = prov_index
df_qlik = pd.merge(left=df_qlik, right=df_mon, how='left', on='N.F')
...
df_qlik['DataColeta'] = df_qlik['DataColeta'].apply(func=lambda val: pd.to_datetime(val, 'coerce'))
df_qlik['DataAgenda'] = df_qlik['DataAgenda'].apply(func=lambda val: pd.to_datetime(val, 'coerce'))
df_qlik['DataEntrega'] = df_qlik['DataEntrega'].apply(func=lambda val: pd.to_datetime(val, 'coerce'))

for x in df_qlik['DataColeta']:
    print(x)
